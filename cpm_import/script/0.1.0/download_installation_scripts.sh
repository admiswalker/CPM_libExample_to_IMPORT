...
baseURL='https://github.com/admiswalker/CPM_libExample_to_IMPORT/raw/main/cpm_import/script/0.1.0'
if [ ! -e $CPM_CACHE_DIR/common_fn.sh               ]; then wget -P $CPM_CACHE_DIR $baseURL/common_fn.sh;               fi
if [ ! -e $CPM_CACHE_DIR/common_fn_envDependents.sh ]; then wget -P $CPM_CACHE_DIR $baseURL/common_fn_envDependents.sh; fi
...
if [ ! -e $CPM_CACHE_DIR/init_path.sh               ]; then wget -P $CPM_CACHE_DIR $baseURL/init_path.sh;               fi
